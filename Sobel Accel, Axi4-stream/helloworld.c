...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	// *IMPORTANT* - flush contents of 'realdata' from data cache to memory
	   ...
	   status = XAxiDma_SimpleTransfer(&axiDma, (unsigned char)*input,
			   5*1024*sizeof(char), XAXIDMA_DMA_TO_DEVICE);

	   do {
	  		   //status = XSobel_filter_IsDone(&xsobel);
	   ...
	   for (i=0; i< 5*1024; i++){
		   status = XAxiDma_SimpleTransfer(&axiDma, (unsigned char)*(output+i) ,
		   			  1*sizeof(char), XAXIDMA_DEVICE_TO_DMA);
	   }


	   printf("kolhsa 26.1 %u\r\n",status);
//
//	   status = XAxiDma_SimpleTransfer(&axiDma, (unsigned char )*(output + 3*1024*8),
//	   			   3*1024*sizeof(char), XAXIDMA_DEVICE_TO_DMA);

	   printf("kolhsa 26.2 %u\r\n",status);
...
//	   	   printf("kolhsa 2\r\n");
//	   	   // DMA enough data to push out first result data set completely
//	   	   status = XAxiDma_SimpleTransfer(&axiDma, (unsigned char)*(input+2*1024),
//	   			   2*1024*sizeof(char), XAXIDMA_DMA_TO_DEVICE);
//	   	   printf("kolhsa 23\r\n");
//	   	   if (status != XST_SUCCESS) { return XST_FAILURE; }
//	   	   printf("kolhsa 24\r\n");
//	   	   //pra3eis
//	   	   // Setup DMA from PL to PS memory using AXI DMA's 'simple' transfer mode
//	   	   status = XAxiDma_SimpleTransfer(&axiDma, (unsigned char )*(output+2*1024),
//	   			   2*1024*sizeof(char), XAXIDMA_DEVICE_TO_DMA);
//
//	   	   printf("kolhsa 26 %u\r\n",status);
//
//	   	   if (status != XST_SUCCESS) { return XST_FAILURE; }
